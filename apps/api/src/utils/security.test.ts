...
describe("token helpers", () => {
  it("generates non-predictable public token values and hashes them", async () => {
    ...
    const { createOpaqueToken, hashToken, signRefreshToken } = await import("./security");
    ...
    expect(signRefreshToken({ userId: "test-user" })).not.toBe(signRefreshToken({ userId: "test-user" }));
  });
});
